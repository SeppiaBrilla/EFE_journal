...
class Model(nn.Module):
    def __init__(self, feature_size:int, output_size:int, final_Activation_function=None) -> None:
        super().__init__()
    ...
    def forward(self, inputs):
        ...
        features = self.features(encoded_input)
        ...
        return torch.concat((features, out), dim=1)

class Language_features_generator:
    def __init__(self, names:'list', pre_trained_weights:'str', feature_size:'int', num_classes:'int') -> None:
        super().__init__()
        self.device = device("cuda:0" if cuda.is_available() else "cpu")
        self.model = Model(feature_size, num_classes, torch.nn.functional.relu)
