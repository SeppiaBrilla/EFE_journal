...
class RegressionModel(nn.Module):
    def __init__(self, feature_size, output_size, activation) -> None:
        super().__init__()
...
def main():
    ...
    assert args.activation == "relu" or args.activation == "sigmoid", f"activation can only be relu or sigmoid. Got: {args.activation}"
    activation = nn.functional.sigmoid if args.activation == "sigmoid" else nn.functional.relu
    ...
    model =  RegressionModel(100, length, activation)
    ...
    for instance in tqdm(train_instances):
        ...
        t = time() - t
        remove(x)
        y_pred = int(torch.argmin(y_pred[0]))
        opt = options[y_pred]
    ...
    for instance in tqdm(validation_instances):
        ...
        t = time() - t
        remove(x)
        y_pred = int(torch.argmin(y_pred))
        opt = options[y_pred]
